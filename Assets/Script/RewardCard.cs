...
public class RewardCard : MonoBehaviour
{
    ...
    public string RewardType; // 카드가 활성화될 때의 스테이지 타입 (예: "Battle", "Reward", "Boss" 등)
    public Sprite Image;
    public string rewardData; // 카드 데이터
    ...
    void OnMouseDown()    //이 카드를 누르면 신기한 일이 일어날 거에요
    {
        ...
        transform.rotation = new Quaternion(0, 0, 0, 0);
        transform.localScale = new Vector3(1.3f, 1.3f, 1);
        RewardManager.CardSelect(this.gameObject);
        //최초로 클릭한 위치를 기억해요.
    ...
    void OnMouseUp()       //이 카드를 손에서 논 순간 마법이 펼쳐져요
    {
        ...
        {
            RewardManager.RewardCardActive(this.gameObject);
            UsedCardRemove();
        }
    ...
    {
        RewardManager.RewardHand.Remove(Name);
        RewardManager.RewardHandObject.Remove(this.gameObject);
